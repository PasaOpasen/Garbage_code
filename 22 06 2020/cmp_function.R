...
)
{
    ...
    ns = colnames(result.real)
    ns.ct = ns[startsWith(ns,'count of')]
    ns.st = ns[startsWith(ns,'stat of')]
    ns.df = ns[startsWith(ns,'diff of')]
    ns.pv = ns[startsWith(ns,'p.value of')]
    
    result.real = cbind(
      result.real[,c(as.vector(rbind(ns.ct,ns.st)),as.vector(rbind(ns.df,ns.pv)))],
    method
      )
